...
class voucher extends FPDF {
    ...
    public function Header() {
            ...
            $this->Cell(20, 10, 'TICKET DE ENTRADA');
            $this->Ln(10);
            ...
            $consulta = mysql_query("SELECT H.dia_desde, H.dia_hasta, H.hora_inicio, H.hora_termino FROM horario H WHERE H.rut_estacionamiento ='" . $rut . "' ");

            while ($datos = mysql_fetch_array($consulta)) {
                $descripcion = $datos['dia_desde'] . " a " . $datos['dia_hasta'];
            ...
            //Colocar fecha y hora de forma dinámica
            $this->Ln(5);
            date_default_timezone_set('America/Santiago');
            $ingreso = date('Y-m-d H:i:s');
            $this->Cell(20, 10, 'ENTRADA ' . $ingreso);
            $this->Ln(5);
            $consulta = "INSERT INTO vehiculo(fecha_ingreso,rut_estacionamiento,estado) VALUES ('$ingreso','$rut','1')";
            $query = "UPDATE estacionamiento SET plazas_disponibles=plazas_disponibles-1 WHERE rut_estacionamiento='$rut'";
...
$mipdf = new voucher();
$mipdf->FPDF('P', 'mm', array(100, 125));
$mipdf->addPage();
$codigo = (int) date('Ymdhi');
$mipdf->EAN13(30, 100, $codigo);
$mipdf->Output();
$mipdf->Close();
?><?php
...
class voucher extends FPDF {
    ...
    public function Header() {
        $consulta = mysql_query("SELECT E.plazas_disponibles FROM estacionamiento E");
        $datos = mysql_fetch_array($consulta);
        $datos1 = mysql_result($datos, 0);
        //$rut = $datos['rut_estacionamiento'];
