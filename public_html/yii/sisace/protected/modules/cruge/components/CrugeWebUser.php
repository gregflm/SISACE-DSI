...
class CrugeWebUser extends CWebUser implements IWebUser
{
    ...
    */
    ...
    {
        if ($this->isGuest) {
            throw new CrugeException("debe iniciar sesión");
        }
    }
    ...
    public function login( /*IUserIdentity*/
        ...
        if (($usersession = $filtro->startSession($user, $system)) != null) {
            ...
            if ($filtro->onStore($usersession)) {
                ...
                Yii::log(CHtml::errorSummary($usersession, "error al guardar una sesión"), "error");
                $this->_lastError = CrugeTranslator::t("Error al almacenar sesión");
                return false;
            }
        } else {
